...
public class Beacon implements java.io.Serializable{
    ...
    public Beacon(int idb, String zona, String map_id, JSONArray vicini, JSONArray vic_dis, Graph graph) {
        this.idb = idb;
        this.zona = zona;
        this.map_id = map_id;
        ...
        if(vicini != null) {
            try {
                ...
                for (int k = 0; k < vicini.length(); k++) {
                    ...
                    graph.addLink(idb,vicini.getJSONObject(k).getInt("beacon_arrivo"),
                            vicini.getJSONObject(k).getInt("gradi"),
                            vicini.getJSONObject(k).getInt("costo"));
                }
            } catch (Exception E) {
